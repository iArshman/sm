...
from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup, BotCommand # <-- Added BotCommand
...
async def on_startup(_):
    """Initialize bot on startup and set bot commands"""
    logger.info("🚀 Bot starting up...")
    
    # Define the commands for the menu
    commands = [
        BotCommand(command="/start", description="🖥️ Main menu / View servers"),
        BotCommand(command="/add_server", description="➕ Add a new server"),
    ...
    try:
        ...
        logger.info("✅ Telegram Bot Command Menu set.")
    except Exception as e:
...
async def start_command(message: types.Message):
    ...
    try:
        ...
        if not servers:
            ...
            # Note: The 'add_server' command is also handled by a message_handler below
...
# --- ADDED HANDLER TO CATCH /add_server command directly ---
@dp.message_handler(commands=['add_server'])
async def add_server_command(message: types.Message):
    """Handle /add_server command directly from menu"""
    # This calls the same logic as the callback_query handler 'add_server'
    await add_server_start(message)
# ------------------------------------------------------------
...
# --- HANDLER FOR /cancel command AND cancel callback ---
@dp.message_handler(commands=['cancel'])
@dp.callback_query_handler(lambda c: c.data == "cancel")
async def cancel_action(input_obj: types.Message | types.CallbackQuery):
    """Cancel current action"""
    
    if isinstance(input_obj, types.CallbackQuery):
        message = input_obj.message
        user_id = input_obj.from_user.id
        # Use try-except for callback deletion
        try:
            await input_obj.message.delete()
        except Exception:
            pass
    else:
        message = input_obj
...
# --- SERVER MANAGEMENT (START ADDING SERVER) ---

@dp.callback_query_handler(lambda c: c.data == "add_server")
async def add_server_start(callback: types.CallbackQuery | types.Message):
    """Start adding a new server (Handles both callback and direct command)"""
    
    user_id = callback.from_user.id
    
    # If called from a message (command), no need to answer the callback query
    if isinstance(callback, types.CallbackQuery):
        await callback.answer()

    user_input[user_id] = {'step': 'name'}
...
async def view_server(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Deployment feature removed, now uses 2x2 layout
        kb.add(
